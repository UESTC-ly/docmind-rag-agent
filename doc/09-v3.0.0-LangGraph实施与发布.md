...
> **v3.1 后续状态**：本节保留 v3.0 发布时的历史边界。v3.1 已实现同一 `run_id` 的
> Redis/SQLite owner-token lease 与定期 checkpoint 清理；共享生产 checkpointer、Generic
> Skill 子图和完整 Multi-Agent 仍未实现。当前契约见 `doc/10-v3.1.0-分布式运行锁与生命周期维护.md`。
